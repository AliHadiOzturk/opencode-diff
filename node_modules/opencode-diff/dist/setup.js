...
function validateNodeVersion() {
    ...
    if (majorVersion < 18) {
        error(`Node.js version ${nodeVersion} is not supported. Please upgrade to Node.js >= 18.0.0`);
    }
    log(`Node.js version: ${nodeVersion} ✓`);
}
function checkPackageJson() {
    if (!existsSync('package.json')) {
        error('No package.json found in current directory. Please run this command in a project directory.');
    }
...
function checkOpenCodeCli() {
    try {
        ...
        warn('OpenCode CLI not found. You may need to install it: npm install -g @opencode-ai/cli');
        return false;
    }
}
function setupOpenCodeJson(dryRun) {
    ...
    if (existsSync(opencodePath)) {
        ...
        const isPluginInstalled = config.plugins.some((plugin) => plugin === PLUGIN_NAME || plugin.startsWith(`${PLUGIN_NAME}@`));
...
function setupPluginConfig(dryRun) {
